...
%================================================================
\section{Results}
The study has two independent axes: (i) a \emph{thickness convergence} sweep over
$h/R=h/W=0.01$ to $0.20$ (five values) from thin to thick, and (ii) a
\emph{reference-surface} comparison (wall mid-surface vs.\ outer mould line). Every
non-zero elastic constant is reported on both axes term by term, at every
thickness and both references --- \textbf{no quantity is averaged or reduced to a
mean}; the figures and tables carry the individual values. The MSG plate
transverse-shear $G$ (the $8\times8$ RM block) is used throughout, with no
...
\begin{figure}[t]\centering
\includegraphics[width=\textwidth]{err_tube_iso.png}
...
(dashed) drifts $O(h/R)$; the analytic closed form (dotted, black) overlays the
centre curves.}
\label{fig:err_tube_iso}
\end{figure}

\begin{figure}[t]\centering
\includegraphics[width=\textwidth]{err_tube_aniso.png}
...
\begin{figure}[t]\centering
\includegraphics[width=\textwidth]{err_strip_iso.png}
...
both (1D-shell limit). OML $EI_2$/$GJ$ are reference offsets (off-scale). The
analytic closed form (dotted, black) overlays the centre curves except for the
2D-limited $GA_3$.}
\label{fig:err_strip_iso}
\end{figure}

\begin{figure}[t]\centering
\includegraphics[width=\textwidth]{err_strip_aniso.png}
...
\subsection{Isotropic verification against analytic closed forms}
For the two isotropic sections every non-zero Timoshenko stiffness has a
closed form, which we tabulate as a column beside the FEniCS-solid and the MSG
shell (RM, KF; centre) at the thin and thick ends of the sweep
(Tables~\ref{tab:tube_iso_analytic_1}--\ref{tab:strip_iso_analytic_20}); the
analytic series is also overlaid on the isotropic error plots
(Figs.~\ref{fig:err_tube_iso},~\ref{fig:err_strip_iso}, dotted). The transverse
shear uses the \emph{MSG plate} transverse-shear stiffness $G=\tfrac56 Gh$ (the
$8\times8$ RM plate $G$ block, which carries no shear-correction factor), not an
ad-hoc beam coefficient.

\paragraph{Circle (tube).} The annulus closed forms
$EA=E\pi(r_o^2-r_i^2)$, $EI=E\tfrac\pi4(r_o^4-r_i^4)$, $GJ=2EI\,G/E$ match the
solid to $<0.05\%$ at \emph{every} $h/R$; the transverse shear (the closed-cell
wall-membrane value $GA=\tfrac12 GA$) is within $1.6\%$ to $h/R=0.20$. The MSG
shell reproduces all of these, confirming the solid is mesh-converged and the
shell is exact in the isotropic limit.

\paragraph{Strip.} $EA=EWh$ and $EI_3=EhW^3/12$ are exact; the exact St-Venant
rectangular torsion $J=Wh^3[\tfrac13-0.21\tfrac hW(1-\tfrac{(h/W)^4}{12})]$ matches
to $<0.15\%$; the in-plane shear $GA_2=\tfrac56 GWh$ (the MSG plate $G$ times the
width) matches to $<0.02\%$. The weak bending $EI_2=EWh^3/12$ is within $-1.9\%$ at
$h/W=0.01$, narrowing as $h/W$ grows --- the residual is the finite-width
anticlastic (plane-strain) effect the 1D beam/shell omits, not an error. The one
term without a usable 1D closed form is $GA_3$ (thickness-direction shear of a wide
strip): the compact $\tfrac56 GWh$ over-predicts the true 2D value by orders of
magnitude at small $h/W$ (marked $\dagger$), the same 2D limitation seen in the
shell.
...
\begin{figure}[t]\centering
\includegraphics[width=\textwidth]{coupl_tube_aniso.png}
...
\begin{figure}[t]\centering
\includegraphics[width=\textwidth]{coupl_strip_aniso.png}
