...
\includegraphics[width=0.86\textwidth]{err_strip_aniso.png}
\caption{$[45/\!-\!45]$ strip: \% error vs.\ the solid (centre = solid lines, OML
= dashed). Note $C_{44}$ ($GJ$): RM flat at zero, KF rising to $+22\%$---RM is more
accurate for thick open-section torsion. $C_{33}$ ($GA_3$) is off for both (1D-shell
limit); $C_{22}$ ($GA_2$) is under-predicted by RM. OML $EI_2$/$GJ$ are off-scale
reference offsets. (Full set of error/coupling plots in \texttt{benchmark\_report.pdf}.)}
\end{figure}

\begin{figure}[p]\centering
\includegraphics[width=0.86\textwidth]{coupl_strip_aniso.png}
\caption{$[45/\!-\!45]$ strip normalized couplings $|c_{ij}|$. $C_{14}$ (ext--twist)
and $C_{25}$ (shear--bend) are the genuine material couplings; $C_{15}$ is the OML
