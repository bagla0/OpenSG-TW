"""
FEniCS 2D-solid (VABS) Timoshenko 6x6 for the isotropic strip over a WIDTH sweep
at FIXED thickness h -- the reference for the strip-width convergence study.
As W grows (h/W -> 0) the section becomes a slender plate and the 1D shell should
converge to the solid.  Run in WSL (opensg_env_v8).  Saves data/strip_width_solid.csv.
...
WORK = "/tmp/stripwidth"; os.makedirs(WORK, exist_ok=True); os.chdir(WORK)
OUTDIR = "/mnt/c/Users/bagla0/OneDrive - purdue.edu/2026_195/Claude_code/benchmark_tube/data"
os.makedirs(OUTDIR, exist_ok=True)

H = 0.05                                   # fixed thickness
WIDTHS = [0.1, 0.2, 0.35, 0.6, 1.0, 1.6, 2.5]     # h/W = 0.5 .. 0.02
NC, NR = 160, 16
...
def main():
    ...
    rows = []
    for W in WIDTHS:
        yml = os.path.join(WORK, f"strip_W{W}.yaml")
        gen_yaml(yml, W)
        sm = SolidBounMesh(yml)
        mp, _ = sm.material_database
        C6 = np.asarray(compute_timo_boun(mp, sm.meshdata)[0])
        rows.append([W] + list(C6.flatten()))
        print(f"[solid] W={W} (h/W={H/W:.3f}): EA={C6[0,0]:.4e} GA2={C6[1,1]:.4e} "
              f"GJ={C6[3,3]:.4e} EI2={C6[4,4]:.4e} EI3={C6[5,5]:.4e}")
    hdr = "W," + ",".join(f"C{i+1}{j+1}" for i in range(6) for j in range(6))
    with open(os.path.join(OUTDIR, "strip_width_solid.csv"), "w") as f:
        f.write(hdr + "\n")
        for r in rows:
            f.write(",".join(str(x) for x in r) + "\n")
    print("wrote", os.path.join(OUTDIR, "strip_width_solid.csv"))
