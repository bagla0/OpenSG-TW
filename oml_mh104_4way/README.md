...
## Plots (absolute stiffness vs f; benchmark comparison)
Each panel: JAX-Kirchhoff (blue ○), JAX-RM (red △), FEniCS-shell (green ◇) — open markers — and the
**FEniCS-solid BENCHMARK** drawn on top as a bold black line with large filled **★ (star)** markers.
Diagonal panels labelled `EA (C11)…`; coupling panels `C14: EA-GJ` etc. Dashed line = **f=0.3**
thin/thick boundary (spar h/H≈0.08).
- `figures/oml_abs_diagonal.png` — all 6 stiffnesses.
- `figures/oml_abs_coupling.png` — the significant couplings C14, C15, C16, C24, C25 (|solid|≥1e7).
- `figures/oml_abs_coupling_small.png` — the remaining (neglected) couplings, |solid|<1e7.
- `individual/` — **one PNG per term** (all 21: `C11_EA.png`, `C14_EA-GJ.png`, …) for dropping
  straight into Overleaf.
- (% error plots intentionally omitted for mh104 — direct/absolute reads better; numbers are in
  `data/timo_full_comparison.txt`.)
