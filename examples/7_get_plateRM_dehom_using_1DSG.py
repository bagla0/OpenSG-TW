...
def six(name):
    def parse(vals):
        if vals is None:
            return np.zeros(6)
        if len(vals) != 6:
            raise argparse.ArgumentTypeError("%s needs exactly 6 numbers" % name)
...
def main():
    ...
    if name not in layup_db:
        raise SystemExit("no laminate %r; available: %s" % (name, ", ".join(names)))
    lay = layup_db[name]
    ...
    if r["G_msg"] is None:
        raise SystemExit("%s: fitted compliance not SPD (degenerate material?)" % name)
