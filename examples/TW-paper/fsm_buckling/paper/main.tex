...
$20\%$; a \emph{multi-harmonic} formulation, in which opposite-parity harmonics couple through those very
terms, recovers it. The method is verified against three-dimensional MITC4 shell eigenvalue analysis on an
axially-compressed cylinder (isotropic and $[\pm45^\circ]$) and on a tapered cone represented by three
cross-sections, establishing the cross-section-based pathway for the full composite blade.
\end{abstract}
...
A single harmonic is exact for isotropic walls (isotropic cylinder within $2\%$ of the shell FEA and closer
than it to the classical value) but silently discards the anisotropic bend--twist coupling $D_{16},D_{26}$,
over-predicting a $[\pm45^\circ]$ tube by $20\%$; the multi-harmonic formulation, in which opposite-parity
harmonics couple through those terms, recovers it and converges to $1.09\times$ the shell FEA by
$M\!\ge\!12$. Mesh refinement shows the remaining anisotropic margin is transverse-shear flexibility, not a
reference artifact---a Reissner--Mindlin strip is the identified route to the $<\!5\%$ target and is
essential for foam-core sandwich panels. On a tapered cone the cone represented by three cross-sections
reproduces the full three-dimensional buckling to within the same margin and correctly localizes the
critical (tip) station, establishing the per-station pathway \eqref{eq:min} for the non-prismatic blade,
where each station contributes its own contour, laminate and dehomogenized pre-buckling stress. Future work
adds the transverse-shear (RM) strip and applies the method across all stations of the IEA-22-280-RWT blade.
