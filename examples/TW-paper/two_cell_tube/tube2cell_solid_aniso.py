"""WSL/FEniCS-2D-solid Timoshenko 6x6 for the ANISOTROPIC [-45] 2-cell tube."""
import os
import sys
import time
import numpy as np
...
t0 = time.time()
for tag in ("aniso_thin", "aniso_thick"):
    ...
    np.savetxt(os.path.join(DATA, "C6_solid_tube2cell_%s.txt" % tag), Cs,
               header="FEniCS-2D-solid 2-cell tube [-45] %s [EA GA2 GA3 GJ EI2 EI3]" % tag)
    print("%-11s %4.0fs:  EA=%.4e GA2=%.4e GA3=%.4e GJ=%.4e EI2=%.4e EI3=%.4e"
