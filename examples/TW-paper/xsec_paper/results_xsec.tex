...
\begin{figure}[htbp]\centering
\includegraphics[width=\textwidth]{figures/dehom_r020_capright.png}
\caption{LP spar-cap through-thickness (a right-of-centre cap column, OML$\to$IML, $88$~mm): RM
shell vs.\ VABS. The path is built from the VABS Gauss points, so every sample is exact in both
models (no interpolation, no lateral weighting); the $1$-D RM recovery reproduces the full
laminate---outer glass facesheets ($\approx\!-4$~MPa), carbon core ($\approx\!-20$~MPa), and
both glass/carbon interfaces---to $0.8\%$ in the in-plane stress.}\label{fig:dehom_r020_cap}
\end{figure}
