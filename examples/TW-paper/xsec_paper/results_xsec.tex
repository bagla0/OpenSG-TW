...
\begin{figure}[htbp]\centering
\includegraphics[width=0.62\textwidth]{figures/dehom_r020_circ_S11.png}
\caption{Circumferential path (LP suction surface, LE$\to$TE), against the non-dimensional path
parameter: the dominant axial stress $\sigma_{11}$ recovered by the RM two-step dehomogenization
(dashed) vs.\ VABS $.\mathrm{SM}$ (solid) overlaps along the entire surface. Each stress component
is stored as its own plot; the remaining ones ($\sigma_{22},\sigma_{12}$ and the transverse
plate-limit terms) are provided individually.}\label{fig:dehom_r020_circ}
\end{figure}

\begin{figure}[htbp]\centering
\includegraphics[width=0.62\textwidth]{figures/dehom_r020_cap_S11.png}
\caption{LP spar-cap through-thickness (a right-of-centre cap column, OML$\to$IML, $88$~mm): the
axial stress $\sigma_{11}$, RM shell (dashed) vs.\ VABS (solid). The path is built from the VABS
Gauss points, so every sample is exact in both models (no interpolation, no lateral weighting); the
$1$-D RM recovery reproduces the full laminate---outer glass facesheets ($\approx\!-4$~MPa), carbon
core ($\approx\!-20$~MPa) and both glass/carbon interfaces---to $0.8\%$ in-plane. The other
components are provided as individual plots.}\label{fig:dehom_r020_cap}
\end{figure}
...
\begin{figure}[htbp]\centering
\includegraphics[width=0.6\textwidth]{figures/r020_disp_circ_u_1.png}
\caption{Axial warping displacement $u_1$ along the LP circumferential path (LE$\to$TE), against
the non-dimensional path parameter: OpenSG RM (dashed) vs.\ VABS $.\mathrm{U}$ (solid), overlapping
along the surface. Each component is stored as an individual plot.}\label{fig:r020_disp_circ}
\end{figure}

\begin{figure}[htbp]\centering
\includegraphics[width=0.6\textwidth]{figures/r020_disp_cap_u_1.png}
\caption{Axial warping $u_1$ through the LP spar-cap thickness (OML$\to$IML): with the director term
$z\,(\omega\times e_3)$ the RM recovery (dashed) declines through the wall in step with VABS (solid),
to a residual $\sim\!5\%$ leading-order offset.}\label{fig:r020_disp_cap}
\end{figure}
