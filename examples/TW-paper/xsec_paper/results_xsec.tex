...
\input{tab_rm/ex2_ell_iso.tex}
\input{tab_rm/ex2_ell_m45.tex}
...
\input{tab_rm/iea_mat.tex}
\input{tab_rm/iea_layup_r020.tex}
\input{tab_rm/iea020.tex}
...
\input{tab_rm/fullblade_real.tex}
...
section at $r=0.2$ (the same blade as Examples~3 and~4; a $7.2$~m chord with three shear
...
The section, its material regions and the two sampling paths---a through-thickness column
through the LP spar cap (a right-of-centre column, built from the VABS Gauss points so every
sample coincides exactly in both models) and a circumferential path along the LP (suction)
surface from the leading to the trailing edge---are shown in Figure~\ref{fig:r020_section}.
...
dehomogenization paths: the LP-surface circumferential path (blue, LE$\to$TE) and the LP spar-cap
through-thickness column (red).}\label{fig:r020_section}
\end{figure}
...
\begin{figure}[htbp]\centering
\includegraphics[width=0.49\textwidth]{figures/dehom_r020_circ_S11.png}\hfill
\includegraphics[width=0.49\textwidth]{figures/dehom_r020_circ_S22.png}\\[1pt]
\includegraphics[width=0.49\textwidth]{figures/dehom_r020_circ_S33.png}\hfill
\includegraphics[width=0.49\textwidth]{figures/dehom_r020_circ_S23.png}\\[1pt]
\includegraphics[width=0.49\textwidth]{figures/dehom_r020_circ_S13.png}\hfill
\includegraphics[width=0.49\textwidth]{figures/dehom_r020_circ_S12.png}
\caption{Circumferential path (LP suction surface, LE$\to$TE) against the non-dimensional path
parameter---all six stress components as individual plots: RM shell two-step dehomogenization (red
dashed) vs.\ VABS $.\mathrm{SM}$ (green solid). The dominant axial $\sigma_{11}$ overlaps VABS along
the entire surface; the transverse $\sigma_{33},\sigma_{13},\sigma_{23}$ are the plate plane-stress
limit.}\label{fig:dehom_r020_circ}
\end{figure}

\begin{figure}[htbp]\centering
\includegraphics[width=0.49\textwidth]{figures/dehom_r020_cap_S11.png}\hfill
\includegraphics[width=0.49\textwidth]{figures/dehom_r020_cap_S22.png}\\[1pt]
\includegraphics[width=0.49\textwidth]{figures/dehom_r020_cap_S33.png}\hfill
\includegraphics[width=0.49\textwidth]{figures/dehom_r020_cap_S23.png}\\[1pt]
\includegraphics[width=0.49\textwidth]{figures/dehom_r020_cap_S13.png}\hfill
\includegraphics[width=0.49\textwidth]{figures/dehom_r020_cap_S12.png}
\caption{LP spar-cap through-thickness (right-of-centre column, OML$\to$IML, $88$~mm)---all six
stress components as individual plots: RM shell (red dashed) vs.\ VABS (green solid). The path is
built from the VABS Gauss points, so every sample is exact in both (no interpolation, no lateral
weighting); the in-plane $\sigma_{11}$ reproduces the full laminate---outer glass ($\approx\!-4$~MPa),
carbon core ($\approx\!-20$~MPa) and both glass/carbon interfaces---to $0.8\%$, while the transverse
$\sigma_{33},\sigma_{13},\sigma_{23}$ sit at the plate plane-stress limit.}\label{fig:dehom_r020_cap}
\end{figure}

The agreement is field-wide, not only on the two paths. The complete recovered field over the full
$2$-D solid section---all node coordinates, the three displacements and all six stress components,
for both VABS and the RM recovery---is exported as \texttt{r020\_dehom\_all.vtk} for any view or
section in ParaView; Figure~\ref{fig:r020_contour} shows the in-plane stress rendered from it, VABS
solid beside the RM shell recovery: the
...
\begin{figure}[htbp]\centering
\includegraphics[width=\textwidth]{figures/pv_r020_S11.png}\\[1pt]
\includegraphics[width=\textwidth]{figures/pv_r020_S22.png}\\[1pt]
\includegraphics[width=\textwidth]{figures/pv_r020_S12.png}
\caption{In-plane stress over the full $2$-D solid section (with the three shear webs), rendered in
ParaView from the exported \texttt{.vtk}: VABS solid (left) vs.\ OpenSG RM (right) for
$\sigma_{11},\sigma_{22},\sigma_{12}$ (individual). The strength-driving $\sigma_{11}$
(compression in the LP cap, tension in the HP cap) matches point-for-point; the smaller components
differ mainly in the web shear flow.}\label{fig:r020_contour}
\end{figure}

Dehomogenization also recovers the full local \emph{displacement} state. Along the LP
circumferential path the recovered warping tracks VABS (Figure~\ref{fig:r020_disp_circ})---the
axial warping $u_1$ within $\sim\!0.05$~mm over the $8$~m surface. Through the thickness the RM
recovery adds the director rotation $z\,(\omega\times e_3)$, so the axial warping decreases from
the OML to the IML in step with the solid (Figure~\ref{fig:r020_disp_cap}; a residual $\sim\!5\%$
offset is the leading-order limit). The agreement is field-wide: Figure~\ref{fig:r020_disp_contour}
contours all three warping components over the whole section, VABS solid beside the RM shell
recovery, and the patterns coincide point-for-point; the total displacement magnitude
$|u|=\sqrt{u_1^2+u_2^2+u_3^2}$ (Figure~\ref{fig:r020_mag}) is likewise reproduced across the
cross-section.

\begin{figure}[htbp]\centering
\includegraphics[width=0.32\textwidth]{figures/r020_disp_circ_u_1.png}\hfill
\includegraphics[width=0.32\textwidth]{figures/r020_disp_circ_u_2.png}\hfill
\includegraphics[width=0.32\textwidth]{figures/r020_disp_circ_u_3.png}
\caption{Warping displacement $u_1,u_2,u_3$ along the LP circumferential path (LE$\to$TE) against the
non-dimensional path parameter, as individual plots: OpenSG RM (red dashed) vs.\ VABS $.\mathrm{U}$
(green solid), overlapping along the surface.}\label{fig:r020_disp_circ}
\end{figure}

\begin{figure}[htbp]\centering
\includegraphics[width=0.32\textwidth]{figures/r020_disp_cap_u_1.png}\hfill
\includegraphics[width=0.32\textwidth]{figures/r020_disp_cap_u_2.png}\hfill
\includegraphics[width=0.32\textwidth]{figures/r020_disp_cap_u_3.png}
\caption{Warping displacement $u_1,u_2,u_3$ through the LP spar-cap thickness (OML$\to$IML), as
individual plots: with the director term $z\,(\omega\times e_3)$ the RM recovery (red dashed)
declines through the wall in step with VABS (green solid), to a residual $\sim\!5\%$ leading-order
offset.}\label{fig:r020_disp_cap}
\end{figure}

\begin{figure}[htbp]\centering
\includegraphics[width=\textwidth]{figures/pv_r020_u1.png}\\[1pt]
\includegraphics[width=\textwidth]{figures/pv_r020_u2.png}\\[1pt]
\includegraphics[width=\textwidth]{figures/pv_r020_u3.png}
\caption{Local warping displacement over the full $2$-D solid section (with the three shear webs),
rendered in ParaView from the exported \texttt{.vtk}: VABS solid (left) vs.\ OpenSG RM (right) for
$u_1,u_2,u_3$ (individual). All three components are reproduced across the whole
cross-section.}\label{fig:r020_disp_contour}
\end{figure}

\begin{figure}[htbp]\centering
\includegraphics[width=\textwidth]{figures/r020_disp_mag.png}
\caption{Total local displacement magnitude $|u|=\sqrt{u_1^2+u_2^2+u_3^2}$ on the distorted
section (deformed by the recovered in-plane warping, exaggerated): VABS solid (left) vs.\ OpenSG RM
shell dehomogenization (right). The combined warping field is reproduced across the whole
cross-section.}\label{fig:r020_mag}
\end{figure}
...
airfoil stations of Example~4. Figures~\ref{fig:span_stress} and~\ref{fig:span_disp} trace the
in-plane stress and the displacement at the section crown across the span (against the normalized
spanwise position $\bar r$): the axial $\sigma_{11}$ recovered by the $1$-D RM solve lies on the
VABS solid recovery at every station ($-2.05$ vs.\ $-2.07$~MPa at $r=0.247$, $+0.15$ vs.\ $+0.15$~MPa
at $r=0.98$), and the local displacement tracks within $\sim\!5\%$ ($3.08$ vs.\ $2.98$~mm at
$r=0.247$ down to $0.11$ vs.\ $0.11$~mm at the tip). A single one-dimensional Reissner--Mindlin
element thus recovers the pointwise three-dimensional stress and displacement of the full composite
blade section, everywhere along the span.

\begin{figure}[htbp]\centering
\includegraphics[width=0.72\textwidth]{figures/span_stress.png}
\caption{Spanwise local in-plane stress at the section crown, recovered by dehomogenization under
each windIO station's own beam load: OpenSG RM shell (dashed) vs.\ VABS $2$-D solid (solid). The
axial $\sigma_{11}$ tracks the solid across the span.}\label{fig:span_stress}
\end{figure}

\begin{figure}[htbp]\centering
\includegraphics[width=0.72\textwidth]{figures/span_disp.png}
\caption{Spanwise local displacement at the section crown (components and magnitude $|u|$),
recovered by dehomogenization: OpenSG RM shell (dashed) vs.\ VABS $2$-D solid (solid). Both track across the
span.}\label{fig:span_disp}
\end{figure}

Assembling the station recoveries into the whole blade, Figure~\ref{fig:blade3d} lofts the six
computed windIO cross-sections along the span and colours the outer surface by each recovered
dehomogenization field, interpolated between adjacent stations---the three warping displacements
$u_1,u_2,u_3$ and the three in-plane stresses $\sigma_{11},\sigma_{22},\sigma_{12}$. The complete
three-dimensional strength picture of the blade is assembled from six one-dimensional
cross-sectional solves, each a fraction of a second.

\begin{figure}[htbp]\centering
\includegraphics[width=0.49\textwidth]{figures/blade3d_u1.png}\hfill
\includegraphics[width=0.49\textwidth]{figures/blade3d_S11.png}\\[1pt]
\includegraphics[width=0.49\textwidth]{figures/blade3d_u2.png}\hfill
\includegraphics[width=0.49\textwidth]{figures/blade3d_S22.png}\\[1pt]
\includegraphics[width=0.49\textwidth]{figures/blade3d_u3.png}\hfill
\includegraphics[width=0.49\textwidth]{figures/blade3d_S12.png}
\caption{Full IEA-22 blade coloured by each recovered dehomogenization field on the outer surface,
lofted from the six windIO cross-sections and interpolated between adjacent stations: warping
displacements $u_1,u_2,u_3$ (left column) and in-plane stresses $\sigma_{11},\sigma_{22},\sigma_{12}$
(right column) under the spanwise-varying beam load.}\label{fig:blade3d}
\end{figure}

The outer surface carries only the thin skin plies; the strength-driving stress lives in the carbon
spar caps through the wall. Lofting the wall \emph{through its thickness}
(Figure~\ref{fig:blade3d_solid}) recovers the full-solid field---the peak through-wall axial stress
traces the spar caps down the blade. The complete field (VABS and RM, all six components) is stored
as \texttt{blade3d\_solid.vtk} for section-by-section inspection in ParaView.

\begin{figure}[htbp]\centering
\includegraphics[width=0.82\textwidth]{figures/blade3d_solid_S11.png}
\caption{Full-solid IEA-22 blade: the wall lofted through its thickness across the six stations, the
surface coloured by the peak through-wall axial stress $\sigma_{11}$---the carbon spar caps carry the
spanwise bending. The complete three-dimensional field is stored as \texttt{blade3d\_solid.vtk}
(arrays \texttt{VABS\_*} and \texttt{RM\_*}) for slicing at any section in
ParaView.}\label{fig:blade3d_solid}
\end{figure}
