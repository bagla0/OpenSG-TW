...
shell against the VABS $.\mathrm{K}$ (section axes) at the three references. The six diagonals
plus the two structurally significant couplings---the extension--edgewise-bending $C_{16}$
(the centroid/shear-centre offset, $-3.6\times10^{9}$) and the bend--bend $C_{56}$---are all
within a few to fifteen percent. The chordwise bending $C_{66}\,(EI_3)$ is the outlier
($+8$ to $+14\%$), the single hardest mode for a one-director shell on this thick carbon cap,
and the reason the full-$6\times6$ Frobenius error sits near $6\%$ rather than sub-percent.
The remaining couplings ($C_{15},C_{23},C_{24},C_{34}$) are three orders below the leading
stiffness and sit at the parallel-axis reference-sensitivity floor: their percentages swing
strongly between the OML, centroidal and IML references (most visibly $C_{15}$), which is
exactly why the comparison is reported at all three.
