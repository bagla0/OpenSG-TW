...
\begin{figure}[htbp]\centering
\includegraphics[width=\textwidth]{figures/dehom_r020_capcentre.png}
\caption{Spar-cap centre through-thickness (OML$\to$IML, $88$~mm): RM shell vs.\ VABS. The
in-plane $\sigma_{11}$ overlaps VABS through the full carbon cap ($\approx\!-21$~MPa);
the endpoint departures are the thin glass facesheets, where the stress transitions over a
few plies.}\label{fig:dehom_r020_cap}
\end{figure}

The in-plane stress---which carries essentially all of the strength-relevant field---is
reproduced along the whole LP surface and through the full $88$~mm carbon cap: the axial
$\sigma_{11}$ recovered by the $1$-D RM solve lies on top of the VABS solid recovery. The
transverse-normal $\sigma_{33}$ and the transverse shears $\sigma_{13},\sigma_{23}$ are three
to four orders of magnitude below the in-plane and are reported at the plate plane-stress
limit; a physical $\sigma_{13},\sigma_{23}$ (an equilibrium shear-flow effect rather than a
local constitutive one) is left to future work and does not affect the strength-relevant
in-plane field. Finally, dehomogenization recovers the full local \emph{displacement} state,
not only stress: Figure~\ref{fig:r020_disp} shows the recovered warping field under the same
load---the out-of-plane axial warping $u_1$ ($\pm1.5$~mm about the section) and the in-plane
deformed shape, with the compliant trailing edge distorting most.

\begin{figure}[htbp]\centering
\includegraphics[width=\textwidth]{figures/r020_disp.png}
\caption{IEA $r=0.2$ local displacement recovered by dehomogenization under the same beam
load: the axial (out-of-plane) warping $u_1$ (left) and the in-plane displacement on the
deformed section (right, magnified).}\label{fig:r020_disp}
\end{figure}
