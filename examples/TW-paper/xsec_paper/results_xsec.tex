...
Homogenizing every span station assembles the blade's complete one-dimensional beam
model---the same full-blade verification performed for the Kirchhoff--Love cross-section
in the companion study \cite{bagla2024asc,camarena2025}, here with the single RM 6-DOF
element throughout. Figure~\ref{fig:span_loft} shows the eight cross-sections lofted
along the beam reference axis, from $r=0.2$ to $0.9$, coloured by wall region as in
Figure~\ref{fig:iea_mesh}.
Table~\ref{tab:fullblade} and Figure~\ref{fig:span} give the RM shell against the 2-D
solid at all eight stations ($r=0.2$ to $0.9$). Extension and both bendings track the
solid within $1.5\%$, $1.4\%$, and $3.9\%$ respectively at \emph{every} station, and the
chordwise shear within $3.3\%$. The flapwise shear and torsion drift from $-5.6\%$ and
$-2.8\%$ inboard to $-12.3\%$ and $-11.2\%$ at the thin outboard stations---the
transverse-shear-flexible walls thin faster than the section contracts, and the
single-director wall progressively under-resolves the through-thickness shear of the
sandwich panels. Each station homogenizes in about one second.
...
\end{minipage}
\caption{The eight IEA-22 blade cross-sections lofted along the beam reference axis
(dotted), from $r=0.2$ to $0.9$, coloured by wall region with the legend at
right.}\label{fig:span_loft}
\end{figure}

\input{tab_rm/fullblade.tex}

\begin{figure}[htbp]\centering
\includegraphics[width=\textwidth]{figures/full_blade_rm_span.png}
\caption{Full IEA-22 blade: RM 6-DOF cross-section diagonal \% error vs.\ the 2-D solid
across the span (left: transverse shear and torsion; right: extension and bending), OML
laminate reference.}\label{fig:span}
\end{figure}
...
%----------------------------------------------------------------------------------
\subsection{Example 5: local stress recovery by dehomogenization}\label{sec:res_dehom}
...
stress from the beam internal loads---the \emph{dehomogenization} step, which is what
makes a beam model usable for strength. We demonstrate it on a genuinely thick section,
station~15 of the BAR-URC blade (a carbon spar cap $\sim\!40$~mm thick; the mesh is shown
in Figure~\ref{fig:st15_mesh}, the constituents and wall laminates in
Tables~\ref{tab:st15_mat} and~\ref{tab:st15_layup}), and compare against VABS.

\begin{figure}[htbp]\centering
\includegraphics[width=\textwidth]{figures/st15_mesh.png}
\caption{BAR-URC station-15 cross-section: 2-D solid mesh coloured by material (left) and
the 1-D RM shell contour (right), both from the same source description.}\label{fig:st15_mesh}
\end{figure}

\input{tab_rm/st15_mat.tex}
\input{tab_rm/st15_layup.tex}

Homogenization comes first: Table~\ref{tab:st15_homo} lists every nonzero $C_{ij}$ of the RM
shell---the same $C^0$ drilling-constrained, MITC-$\gamma_{23}$ ring used everywhere else in
this paper, at the OML reference---against the VABS $.\mathrm{K}$. Extension, edgewise bending
and the structurally significant couplings---the extension--edgewise-bending $C_{16}$ (the
centroid/shear-centre offset, $-3.6\times10^{9}$) and the bend--bend $C_{56}$---are within a
few to twenty percent. Two diagonals are the outliers on this genuinely thick section: the
transverse shear $C_{33}\,(GA_3)$ at $-13\%$ and the chordwise bending $C_{66}\,(EI_3)$ at
$+14\%$, both the single-director-shell limit on the $\sim\!40$~mm carbon cap, and the reason
the full-$6\times6$ Frobenius error sits near $6\%$ rather than sub-percent. The smallest
coupling $C_{15}$ is three orders below the leading stiffness and near the numerical floor, so
its percentage is not meaningful. Because the dehomogenization below inverts \emph{this} RM
$6\times6$, reporting it here (rather than a Kirchhoff shell's) is what keeps the two steps
consistent.

\input{tab_rm/st15_homo.tex}
...
$\Sigma(\zeta_3)=C(\zeta_3)\,[\mathbf{B}(\zeta_3)V_0+G_e(\zeta_3)]\,\bar{\epsilon}$. We sample
two paths under the station-15 loads: a through-thickness column at the LP spar-cap centre
(Figure~\ref{fig:st15_path}) and a circumferential loop around the section.
Figures~\ref{fig:dehom_cap} and~\ref{fig:dehom_circ} compare the recovered stress against
the VABS $.\mathrm{SM}$ point cloud.

\begin{figure}[htbp]\centering
\includegraphics[width=\textwidth]{figures/st15_path.png}
\caption{Dehomogenization paths on the station-15 section. (a)~The circumferential loop
(blue) and the LP spar-cap through-thickness column (red) drawn as line arrows on the full
section. (b)~The spar-cap laminate zoom, coloured by material, with the through-thickness
recovery arrow running from the outer (OML) to the inner (IML) mould line.}\label{fig:st15_path}
\end{figure}

\begin{figure}[htbp]\centering
\includegraphics[width=\textwidth]{figures/dehom_st15_capcentre.png}
\caption{Spar-cap centre through-thickness: RM shell two-step dehomogenization vs.\ VABS.
The in-plane $\sigma_{11},\sigma_{22},\sigma_{12}$ track VABS to under $1\%$ (peak
$\sigma_{11}=-167$~MPa); the transverse components $\sigma_{33},\sigma_{13},\sigma_{23}$
(note the axis scale, $\lesssim\!0.04$~MPa) are the plate plane-stress
limit.}\label{fig:dehom_cap}
\end{figure}

\begin{figure}[htbp]\centering
\includegraphics[width=\textwidth]{figures/dehom_st15_circumferential.png}
\caption{Circumferential path around the section: the recovered in-plane stress tracks
VABS everywhere except at the discrete web-junction arc positions, where the 1-D shell
reference folds.}\label{fig:dehom_circ}
\end{figure}

The in-plane stress---which carries essentially all of the strength-relevant field on this
skin-and-cap section---is reproduced to under $1\%$ both through the thickness and around
the contour, at the cost of the 1-D solve. The transverse-normal $\sigma_{33}$ and the
transverse shears $\sigma_{13},\sigma_{23}$ are three to four orders of magnitude below the
in-plane at this station and are reported here at the plate plane-stress limit. We note that
although the RM warping does carry the wall transverse-shear strains $2\gamma_{13},
2\gamma_{23}$, the local \emph{constitutive} recovery $\sigma_{13}=G_{13}\gamma_{13}$ does
not reproduce the cap field: in a spar cap the transverse shear is an equilibrium quantity
(set by the arc-gradient of the bending stress), so a single-director shell's fixed
parabolic profile cannot match the sign-changing through-thickness field. A physical
$\sigma_{13},\sigma_{23}$ recovery requires the equilibrium shear-flow around the contour and
is left to future work; it does not affect the strength-relevant in-plane result reported
here. The one remaining caveat is geometric: a path must run along the shell's
through-thickness direction; a column driven through a cap/web \emph{corner}, where the 1-D
reference folds, samples the arc rather than the thickness and must instead be taken at the
cap interior.
