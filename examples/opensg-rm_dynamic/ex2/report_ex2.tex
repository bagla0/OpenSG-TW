...
\end{equation}
The \emph{step} is a suddenly applied constant pressure that is
\emph{removed} at $t_1=6$ ms (after removal the plate vibrates freely);
the \emph{explosive blast} is an impulsive over-pressure decaying
exponentially, the classic idealization of a far-field detonation.  Both
have homogeneous initial conditions.
...
\section{The Khdeir--Reddy formulation and its implementation}
\label{sec:kr}
Reddy's displacement field with $c_1=4/(3h^2)$:
\begin{equation}
u=u_0+z\phi_x-c_1z^3(\phi_x+w_{,x}),\quad
v=v_0+z\phi_y-c_1z^3(\phi_y+w_{,y}),\quad w=w(x,y,t),
\end{equation}
gives in-plane strains $\varepsilon=\varepsilon^0+z\kappa^1+z^3\kappa^3$
with
$\kappa^3=-c_1\{\phi_{x,x}{+}w_{,xx};\ \phi_{y,y}{+}w_{,yy};\
\phi_{x,y}{+}\phi_{y,x}{+}2w_{,xy}\}$
and the parabolic transverse shear
$\gamma(z)=(1-3c_1z^2)\,(\phi+\nabla w)$, which vanishes at the free faces
--- the reason the theory needs no shear-correction factor.  The laminate
enters through the moments
$(A,B,D,E,F,H)=\int \bar Q(z)\,(1,z,z^2,z^3,z^4,z^6)\,dz$ and the
collapsed shear matrix $\int \bar Q_s(1-3c_1z^2)^2dz$, and the inertia
moments $I_k=\int\rho z^k dz$, $k=0\ldots6$ (the consistent mass couples
$u_0$, $\phi_x$ and $w_{,x}$ through $I_3\ldots I_6$).

For SS-1 cross-ply plates the Navier expansion (single mode $m=n=1$,
$\alpha=\pi/a$, $\beta=\pi/b$)
\begin{equation}
(u_0,v_0,w,\phi_x,\phi_y)=
\big(U\cos\alpha x\sin\beta y,\ V\sin\alpha x\cos\beta y,\
W\sin\alpha x\sin\beta y,\ X\cos\alpha x\sin\beta y,\
Y\sin\alpha x\cos\beta y\big)
\end{equation}
satisfies all equations and edges exactly, reducing the plate to
$M\ddot d+Kd=f(t)$ with $d=(U,V,W,X,Y)$, $f=(0,0,q_0F(t),0,0)$ ---
$5\times5$ matrices assembled from the laminate moments.  Each modal
coordinate then has the \emph{closed-form} response ($\omega_i$ from the
generalized eigenproblem $K\phi=\omega^2M\phi$, $g_i=\phi_{w,i}q_0$):
\begin{align}
\text{step, }t\le t_1:&\ \ \eta_i=\tfrac{g_i}{\omega_i^2}(1-\cos\omega_it);
\qquad t>t_1:\ \ \eta_i=\tfrac{g_i}{\omega_i^2}
\big[\cos\omega_i(t{-}t_1)-\cos\omega_it\big],\\
\text{blast}:&\ \ \eta_i=\tfrac{g_i}{\omega_i^2+\gamma^2}
\Big[e^{-\gamma t}-\cos\omega_it+\tfrac{\gamma}{\omega_i}
\sin\omega_it\Big].
\end{align}
This \emph{is} the KR method (their state-space integral evaluates to the
same expressions).  Our implementation
(\code{ex2/reddy\_hsdt\_navier.py}, listed in
Appendix~\ref{app:code}) is \textbf{pinned to Nayak's own converged FE}:
his Table~3 (9-node element, $4\times4$ mesh, $\Delta t=40\,\mu$s) vs.\ our
closed form at his sample times, $w/0.0254$~m:

\begin{center}\begin{tabular}{lccccc}\toprule
$t$ [ms] & 1.6 & 3.2 & 4.8 & 6.4 & 8.0\\\midrule
Nayak Table 3 (9-node FE) & 0.1870 & 0.6180 & 0.9983 & 0.4548 & $-$0.2191\\
this code (exact TSDT)    & 0.2011 & 0.6540 & 1.0225 & 0.3737 & $-$0.3477\\
\bottomrule\end{tabular}\end{center}

\noindent 2--7\,\% at the pre-cut-off times --- his finite $\Delta t$ and
mesh bias (his own convergence study moves toward our values as
$\Delta t\!\downarrow$); the post-cut-off points are phase-sensitive
(Sec.~\ref{sec:results}).  His 4-node Table~2 is still shear-locked and
must not be used as an anchor.
...
\appendix
\section{The exact-HSDT code}
\label{app:code}
\lstinputlisting[style=py]{reddy_hsdt_navier.py}
\end{document}
