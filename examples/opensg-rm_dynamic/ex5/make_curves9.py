...
Stations and depths (matched between the models):
  ...
  S11, S22, S12 center, top-ply centroid depth z = h/2 - tply/2 (the
                solid's outermost stress sample).  S12 = 0 at the center
                by symmetry -- annotated like U1/U2.
  S13           (0, b/2), mid-core;   S23  (a/2, 0), mid-core.
  S33           center, mid-core, by the DYNAMIC MOMENTUM integral
                (rho w_dd + shear divergence), per increment.
...
def run(kind):
    ...
    for setname, with_fs in (("with_FSDT", True), ("without_FSDT", False)):
        ...
        # stresses
        SPEC = [("S11", s11, "S11", r"$\sigma_{11}(a/2,b/2,"
                 r"h/2{-}t_{ply}/2)$ [MPa]", None),
                ("S22", s22, "S22", r"$\sigma_{22}$, same point [MPa]",
                 None),
                ("S12", s12, "S12", r"$\sigma_{12}$, same point [MPa]",
