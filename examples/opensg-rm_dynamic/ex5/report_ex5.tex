...
\paragraph{Full-field deliverables.}  Beyond the station histories, the
suite renders whole-field pictures: the applied double-sine pressure over
the geometry (\code{sandwich\_applied\_load.png}), the benchmark solid's
nine component contours (\code{sandwich\_SOLID\_step\_\{U1..U3,
S11..S23\}.png}, clean image + colorbar, global frame, no mesh lines), and
--- from the 2-D shell \code{.dat} alone --- the standalone
\code{rm\_field\_vtk.py}, which dehomogenizes ALL 441 plate nodes at 44
through-thickness stations (linear-operator precompute + \code{jax.vmap}
over nodes) and writes \code{sandwich\_rm\_field.vtk} with the
displacement vector and all six stresses for 3-D contouring in ParaView.
