...
ax.plot(circ[:, 0], circ[:, 1], '-', color='k', lw=2.4, label='circumferential (LP OML, LE→TE)')
ax.plot(col[:, 0], col[:, 1], '-o', color='magenta', ms=4, lw=2.2, label='lp_sparcap_left (thickness)')
ax.plot([0], [0], 'w+', mec='k', mew=1.5, ms=12)                      # (0,0) reference axis
ax.set_aspect('equal'); ax.autoscale(); ax.axis('off')
lg1 = ax.legend(loc='lower right', fontsize=10, framealpha=0.95)      # paths -> bottom-right, clear of curves
ax.add_artist(lg1)
ax.legend(handles=handles, loc='upper right', fontsize=9, title='materials', framealpha=0.95)
fig.tight_layout()
