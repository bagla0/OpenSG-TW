...
SRC = {
    "rm":     ("homo_rm",     "OpenSG_RM_",     "OpenSG_RM_Shell",
               "Timoshenko 6x6 -- RM SHELL cross-section (OpenSG-RM, Boundary ring, MITC gamma_23)"),
    "jax":    ("homo_jax",    "OpenSG_JAX_",    "OpenSG_JAX_Solid",
               "Timoshenko 6x6 -- 2-D SOLID cross-section (OpenSG-JAX, JAX-FEM Mechanics of Structure Genome)"),
    "fenics": ("homo_fenics", "OpenSG_FEniCSx_", "OpenSG_FEniCSx_Solid",
...
def emit(key, folder, prefix, suffix, label, outdir):
    ...
    sub = os.path.join(outdir, suffix)          # one folder per OpenSG source
    ...
    for fp in files:
        ...
        tt = times.get(tag)
        with open(os.path.join(sub, "%s_%s.out" % (tag, suffix)), "w") as f:
            f.write("# %s\n" % label)
            ...
            f.write("\n")
            mat(f, "Compliance", S)
            mp = os.path.join(HERE, "homo_fenics", "OpenSG_Mass_%s.txt" % tag)   # 6x6 mass matrix (VABS .K style)
            if os.path.exists(mp):
                f.write("\n")
                mat(f, "Mass      ", np.loadtxt(mp))
