...
def _strip_inp(path, title, header_lines, material_lines, section_lines, a, q0, nel,
    ...
    A("U")
    A("** the whole span row: the harmonic amplitudes (U, V, W, F1, F2) of the")
    A("** plate solution are least-squares fitted from these for the Eq.-65")
    A("** displacement recovery (examples/pagano_recovery/recovery_bench.py)")
    A("*NODE PRINT, NSET=NROW0")
    A("U")
