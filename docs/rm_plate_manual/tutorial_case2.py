"""OpenSG-RM Tutorial 2 -- sandwich [0/core/0] plate, S = a/h = 10.

Standalone: run it from anywhere inside the repository and it produces
    tutorial_case2_sg.yaml / _sg.png      the 1-D SG and its mesh figure
    tutorial_case2_outofplane.png         sigma_13, sigma_33 profiles
    tutorial_case2_inplane.png            sigma_11, sigma_22, sigma_12
    tutorial_case2_disp.png               U1, U2, U3
...
User options (PreVABS-style):
    ...
    --model 1    RM shear-refined 8x8 (default): full OpenSG-RM law + the
                 second-order and face-pressure recovery
    --FF N11 N22 N12 M11 M22 M12 Q1 Q2
                 dehomogenize from YOUR resultants instead of the built-in
                 closed-form plate solution of this benchmark; the exact-3-D
                 comparison is skipped (an arbitrary FF has no exact twin)

Problem (default run): simply supported strip, span a = 1 m along x1,
thickness h = a/10, top-face pressure q(x) = q0 sin(pi x/a), q0 = 10 kPa.
Stiff carbon faces (0.1 h each) over a soft core (0.8 h), all plies at
0 deg, mid-surface reference -- the layup where the soft core makes the
transverse shear dominant.
"""
import argparse
import os
import sys
...
# ---------------------------------------------------------------------------
MATERIAL_DB = {"face": {"E": [131.0e9, 10.34e9, 10.34e9],
...
yml = os.path.join(HERE, "tutorial_case2_sg.yaml")
...
#    built-in single-harmonic closed form of this benchmark strip
# ---------------------------------------------------------------------------
p = np.pi / a
S6 = np.linalg.inv(A6)
z6 = np.zeros(6)
if args.FF is not None:
    ...
    Es = S6 @ FF[:6]                       # plate strains from YOUR resultants
    dE1 = S6 @ np.array([0, 0, 0, FF[6], 0, 0.0])       # dM11/dx = Q1
    dE2 = S6 @ np.array([0, 0, 0, 0, FF[7], 0.0])       # dM22/dy = Q2
    dE11 = z6
    qt_e = qt_m = None                     # user load: no face-pressure ladder
    y5 = np.zeros(5)
    compare_exact = False
...
# ---------------------------------------------------------------------------
# 3. dehomogenization through the thickness at the two stations
# ---------------------------------------------------------------------------
zc = np.linspace(-h / 2 + 1e-9, h / 2 - 1e-9, 201)
S_end = np.array([msgrm_strain_at_depth(r, z, z6, dE1, dE2, qt6=qt_e)[1]
                  for z in zc])                            # x = 0 station
S_mid = np.array([msgrm_strain_at_depth(r, z, Es, dE11=dE11, qt6=qt_m)[1]
                  for z in zc])                            # x = a/2 station
s33 = np.concatenate([[0.0], np.cumsum(0.5 * p * (S_end[1:, 4] + S_end[:-1, 4])
                                       * np.diff(zc))])    # equilibrium route
U = np.empty((len(zc), 3))
for i, z in enumerate(zc):
    w0 = msgrm_warping_at_depth(r, z, z6, dE1, dE2, qt6=qt_e)
    wm = msgrm_warping_at_depth(r, z, Es, dE11=dE11, qt6=qt_m)
...
if compare_exact:
    ...
    n11 = np.linalg.norm(np.interp(zc, ze, sige[:, 0]))
    print("\nrecovery errors vs exact 3-D (rel L2 %):")
    print("  s13 %6.2f   s33 %6.2f" % (rel(S_end[:, 4], sige[:, 4]),
                                       rel(s33, sige[:, 2])))
    print("  s11 %6.2f   s22 %6.2f   s12 %s" %
          (rel(S_mid[:, 0], sige[:, 0]), rel(S_mid[:, 1], sige[:, 1]),
           "machine zero" if np.max(np.abs(S_mid[:, 5])) < 1e-8 * n11 else
           "%6.2f" % rel(S_mid[:, 5], sige[:, 5], 1e-2 * n11)))
    print("  U1  %6.2f   U3  %6.2f" % (rel(U[:, 0], uvwe[:, 0]),
                                       rel(U[:, 2], uvwe[:, 2])))
...
for fname, specs in (
    ("outofplane", [(S_end[:, 4], exs[:, 4], r"$\sigma_{13}$ [Pa] at $x=0$"),
                    (s33, exs[:, 2], r"$\sigma_{33}$ [Pa] at $x=a/2$")]),
    ("inplane", [(S_mid[:, 0], exs[:, 0], r"$\sigma_{11}$ [Pa] at $x=a/2$"),
                 (S_mid[:, 1], exs[:, 1], r"$\sigma_{22}$ [Pa] at $x=a/2$"),
                 (S_mid[:, 5], exs[:, 5], r"$\sigma_{12}$ [Pa] at $x=a/2$")]),
    ("disp", [(U[:, 0], exu[:, 0], r"$U_1$ [m] at $x=0$"),
              (U[:, 1], exu[:, 1], r"$U_2$ [m] at $x=0$"),
              (U[:, 2], exu[:, 2], r"$U_3$ [m] at $x=a/2$")])):
    ...
    fig.tight_layout()
    fig.savefig(os.path.join(HERE, "tutorial_case2_%s.png" % fname), dpi=150,
                bbox_inches="tight")
    plt.close(fig)
print("\nwrote tutorial_case2_{sg,outofplane,inplane,disp}.png")

# ---------------------------------------------------------------------------
# 5. failure indices from the recovered ply-frame stress (BOTH criteria)
#    Allowables below are EXAMPLE carbon/epoxy values -- replace with yours.
# ---------------------------------------------------------------------------
Xt, Xc = 1035e6, 689e6          # fiber tension / compression [Pa]
Yt, Yc = 41e6, 117e6            # transverse tension / compression
S12a, S13a, S23a = 69e6, 62e6, 62e6
...
print("  max-stress index = %.4f   Tsai-Wu index = %.4f   (>= 1 = failure)"
      % (ms_worst, tw_worst))
