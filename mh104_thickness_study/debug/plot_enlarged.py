...
def grid(terms, mode, fname, title):
    ...
    for k, (i, j) in enumerate(terms):
        ...
        if mode == "abs":
            ...
            # FEniCS-solid = benchmark: bold black, large FILLED STAR markers, drawn on top
...
grid([(i, j) for (i, j) in od if not keep(i, j)], "abs", "oml_abs_coupling_small.png",
     "mh104 OML SMALL couplings (|solid|<1e7, omitted from the main plot): " + tag)
